...
#ifdef damperSplineGain
      force *= (Interpolation::CatmullSpline(damperSplinePoints[0], damperSplinePoints[1], damperSplineNumPoints, abs(force))
                / 10000.0);
#else
...
void Joystick_::forceCalculator(int32_t* forces) {
  ...
  if (DynamicHID().pidReportHandler.deviceState == MDEVICESTATE_SPRING) {
    ...
    for (int id = 0; id < MAX_EFFECTS; id++) {
      ...
      if ((effect.totalDuration == USB_DURATION_INFINITE) || (effect.elapsedTime < effect.totalDuration)) {
      ...
      if ((effect.state == MEFFECTSTATE_PLAYING) &&
          ...
          //TODO: this is always true due to uint
